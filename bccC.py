    ...
    def _forEachSample(self, data, k, h, verbose):  ## siendo h el numero de muestra
        ...
        for i in range(k):  # for each cluster  ## Si se buscaron 3 clusters, hay un i por cada uno de ellos
            ...
            if ids_Mk.size != 0:
                ...
                # https://stackoverflow.com/questions/20583381/building-and-updating-a-sparse-matrix-in-python-using-scipy
                # siendo Mkh coo_matrix

                cooTmp = coo_matrix((np.ones(ids_Mk[0].shape[0]), (ids_Mk[0], ids_Mk[1])), shape=((data.shape[0],) * 2))
                print("post cooTmp")
                print("cooTmp: ", getsizeof(cooTmp))
                ...
                print("Mkh: ", getsizeof(Mkh))

                # https://stackoverflow.com/questions/27770906/why-are-lil-matrix-and-dok-matrix-so-slow-compared-to-common-dict-of-dicts/27771335#27771335
                # siendo Mkh dok_matrix
                # Mkh.update({(tuple(ids_Mk[0]), tuple(ids_Mk[1])):1}) no funciona
